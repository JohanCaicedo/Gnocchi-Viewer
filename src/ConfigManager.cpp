...
AppConfig ConfigManager::loadConfig(const std::string& filepath) {
    ...
    while (std::getline(file, line)) {
        ...
        try {
            ...
            else if (key == "captureBackend") config.captureBackend = std::stoi(value);
            ...
            else if (key == "showFpsViewer") config.showFpsViewer = (value == "1" || value == "true");
...
void ConfigManager::saveConfig(const std::string& filepath, const AppConfig& config) {
    ...
    file << "captureBackend=" << config.captureBackend << "\n";
    ...
    file << "showFpsViewer=" << (config.showFpsViewer ? "1" : "0") << "\n";
